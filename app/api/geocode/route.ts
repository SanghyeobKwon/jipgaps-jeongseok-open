...
export async function GET(request: Request) {
  ...
  if (!clientId || !clientSecret) {
    return Response.json({ error: "네이버 지도 API가 아직 연결되지 않았습니다." }, { status: 503 });
  }
  ...
  try {
    ...
    if (!address?.x || !address?.y) {
      return Response.json({ error: "선택한 단지의 좌표를 찾지 못했습니다." }, { status: 404 });
    }
    ...
    const message =
      status === "401" || status === "429"
        ? "네이버 클라우드에서 Maps·Geocoding 권한과 Web 서비스 URL을 확인해주세요."
        : "단지 위치를 확인하지 못했습니다.";
