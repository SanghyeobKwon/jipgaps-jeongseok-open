...
export async function GET(request: Request) {
  ...
  const query = `[out:json][timeout:18];(nwr(around:1500,${lat},${lng})["name"]["amenity"~"school|kindergarten|university|college|hospital|clinic|doctors|pharmacy|marketplace|library"];nwr(around:1500,${lat},${lng})["name"]["shop"~"supermarket|convenience|mall|department_store"];nwr(around:1500,${lat},${lng})["name"]["leisure"~"park|fitness_centre|sports_centre|playground"];nwr(around:1500,${lat},${lng})["name"]["railway"~"station|subway_entrance"];nwr(around:1500,${lat},${lng})["name"]["public_transport"="station"];);out center tags;`;
  try {
    const url = new URL("https://overpass-api.de/api/interpreter"); url.searchParams.set("data", query);
    const response = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "JipgapsJeongseok/1.0" }, signal: AbortSignal.timeout(22000) });
    if (!response.ok) throw new Error(`OVERPASS_${response.status}`);
    const data = await response.json() as { elements?: OverpassElement[] }; const seen = new Set<string>();
    const places = (data.elements || []).flatMap((element) => {
      ...
      const distance = Math.round(haversine(lat, lng, placeLat!, placeLng!)); const group = category(tags); const key = `${group}|${name}`; if (distance > 1500 || seen.has(key)) return []; seen.add(key);
    ...
    return Response.json({ places, radius: 1500, source: "OpenStreetMap contributors", measuredAt: new Date().toISOString() }, { headers: { "Cache-Control": "public, s-maxage=86400, stale-while-revalidate=604800" } });
  } catch {
