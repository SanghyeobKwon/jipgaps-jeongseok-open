...
const MARKETS = [
  ["서울", "서울특별시", "11680"], ["부산", "부산광역시", "26350"], ["대구", "대구광역시", "27260"],
  ["인천", "인천광역시", "28200"], ["광주", "광주광역시", "29155"], ["대전", "대전광역시", "30200"],
  ["울산", "울산광역시", "31140"], ["세종", "세종특별자치시", "36110"], ["경기", "경기도", "41135"],
  ["강원", "강원특별자치도", "42150"], ["충북", "충청북도", "43113"], ["충남", "충청남도", "44131"],
  ["전북", "전북특별자치도", "45111"], ["전남", "전라남도", "46130"], ["경북", "경상북도", "47113"],
  ["경남", "경상남도", "48123"], ["제주", "제주특별자치도", "50110"],
] as const;
...
function months() {
  const now = new Date();
  return [1, 0].map((back) => { const date = new Date(now.getFullYear(), now.getMonth() - back, 1); return `${date.getFullYear()}${String(date.getMonth() + 1).padStart(2, "0")}`; });
}
...
export async function GET(request: Request) {
  try {
    ...
    const type = requested as PropertyType; const [previousMonth, currentMonth] = months(); const results = [];
    for (let index = 0; index < MARKETS.length; index += 5) {
      const batch = await Promise.all(MARKETS.slice(index, index + 5).map(async ([short, sido, code]) => {
        const [previous, current] = await Promise.all([fetchMarket(type, code, previousMonth), fetchMarket(type, code, currentMonth)]);
        const before = previous.length ? median(previous) : 0; const latest = current.length ? median(current) : 0;
        return { short, sido, code, count: current.length, median: latest, change: before && latest ? (latest / before - 1) * 100 : 0 };
      })); results.push(...batch);
    }
    return Response.json({ markets: results, month: currentMonth, previousMonth, type }, { headers: { "Cache-Control": "public, s-maxage=21600, stale-while-revalidate=86400" } });
