...
type NaverMapInstance = { data: NaverDataLayer; destroy?: () => void; fitBounds?: (bounds: unknown, margin?: number | { top: number; right: number; bottom: number; left: number }) => void; getZoom?: () => number; setCenter?: (center: unknown) => void; setZoom?: (zoom: number) => void };
...
const NATIONAL_LABEL_OFFSETS: Record<string, [number, number]> = {
  "서울특별시": [27, -12], "인천광역시": [-28, 9], "세종특별자치시": [-24, -10], "대전광역시": [26, 10],
    ...
    {data ? <svg className="administrative-map-svg" viewBox="0 0 760 560" role="img" aria-label={`${stageTitle} 3D 행정구역 선택 지도`}>
      ...
      {boundaries.map((boundary) => {
        ...
        return <g key={boundary.code} className={`administrative-region tone-${boundaryTone(boundary)}${selected ? " selected" : ""}`} role="button" tabIndex={0} aria-label={`${boundary.name} ${boundaryMetric(boundary)}`} onClick={() => selectBoundary(boundary)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); selectBoundary(boundary); } }}>
          <path className="administrative-region-depth" d={boundary.path} transform={`translate(0 ${selected ? 11 : 8})`} />
          <path className="administrative-region-surface" d={boundary.path} fillRule="evenodd" />
          {(offset[0] !== 0 || offset[1] !== 0) && <line className="administrative-label-line" x1={boundary.centerX} y1={boundary.centerY} x2={labelX} y2={labelY} />}
          <g className="administrative-label" transform={`translate(${labelX} ${labelY})`}><text className="administrative-label-name" textAnchor="middle" y="-2">{focus === "national" ? markets.find((item) => item.sido === boundary.name)?.short || boundary.name : boundary.name}</text><text className="administrative-label-metric" textAnchor="middle" y="12">{boundaryMetric(boundary)}</text></g>
        </g>;
      })}
...
function NaverMarketMap({ markets, focus, active, propertyType, selectedSido, activeRegion, selectedDong, selectedBoundaryDong, dongStats, dongMetric, onDongMetricChange, buildingLocations, buildingsLoading, buildingsError, propertyLocation, propertyName, onSelectSido, onSelectRegion, onSelectDong, onSelectProperty }: {
  ...
  useEffect(() => {
    ...
    loadNaverMap(clientId).then(async () => {
      ...
      const fitCollection = (data: GeoJsonFeatureCollection, zoomOut = 0) => {
        ...
        if (zoomOut) window.setTimeout(() => { if (!disposed && map?.getZoom && map.setZoom) map.setZoom(Math.max(6, map.getZoom() - zoomOut)); }, 80);
      };
      ...
      if (focus === "buildings" && map) {
        ...
        buildingLocations.forEach((building) => {
          const typeMeta = PROPERTY_MAP_META[building.propertyType] || PROPERTY_MAP_META[propertyType];
          const marker = new maps.Marker({
            ...
            icon: { content: `<button class="naver-building-pin kind-${building.propertyType}" type="button" aria-label="${escapeMapHtml(building.name)} ${escapeMapHtml(formatPrice(building.lastAmount))}"><strong>${escapeMapHtml(formatPrice(building.lastAmount))}</strong><span>${escapeMapHtml(typeMeta.short)} · ${building.count}건</span></button>`, anchor: new maps.Point(18, 38) },
...
export default function Home() {
  ...
  const chooseMapDong = useCallback((dong: string) => { const legalDong = legalDongName(dong); setSelectedBoundaryDong(dong); setMapPickerDong(dong); setMapFocus("buildings"); resetPropertySelection(); setSelectedDong(legalDong || dong); }, [resetPropertySelection]);
  ...
  const mapSelectorTitle = mapFocus === "national" ? "전국 시·도 선택" : mapFocus === "sido" ? `${selectedMapSido} 시·군·구 선택` : `${activeRegion.sigungu} 읍·면·동 선택`;
  const mapSelectorDescription = mapFocus === "national" ? "16개 시·도 중 한 곳을 선택하면 해당 시·도 전체 지도로 이동합니다." : mapFocus === "sido" ? "시·군·구를 선택하면 해당 지역의 동 경계 지도로 바로 이동합니다." : "동을 선택하면 실제 지도에서 거래 건물과 가격을 확인합니다.";
  ...
  return <main className="terminal-shell" data-view={activeSection}>
    ...
    <section className="map-section" id="map">
      <div className="map-current-location" aria-live="polite"><div><span>현재 보고 있는 범위</span><strong>{mapLocationTitle}</strong><small>{mapLocationDescription}</small></div><button onClick={() => { changeView("home"); window.setTimeout(() => document.querySelector<HTMLSelectElement>('.search-console select')?.focus(), 250); }}>지역 다시 선택</button></div>
      <div className="section-title wide"><div><p>KOREA MARKET ATLAS</p><h2>입체 행정경계에서 실제 생활권까지</h2><span>{marketMonth ? `${marketMonth.slice(0, 4)}년 ${Number(marketMonth.slice(4))}월 기준 최근 3개월` : "전국 집계 중"} · 전국·시도·구·동은 3D 행정지도로 탐색하고, 동 선택 후 실제 지도로 평가합니다.</span></div><div className="map-controls"><button type="button" className="gangnam-map-shortcut" onClick={openGangnamMap}>강남구 바로보기 ↗</button><button type="button" className="gangnam-map-shortcut secondary" onClick={openHaengdangMap}>행당동 예시보기 ↗</button><select value={marketSort} onChange={(event) => setMarketSort(event.target.value as typeof marketSort)} aria-label="전국 시장 정렬"><option value="volume">최근 3개월 거래량순</option><option value="price">최근 3개월 중위가순</option><option value="rise">직전 3개월 대비 상승순</option><option value="fall">직전 3개월 대비 하락순</option></select><div className="map-legend"><i className="cold"/>하락 <i className="flat"/>보합 <i className="hot"/>상승</div></div></div>
      <div className="map-direct-picker" aria-label="목록으로 지역 선택">
        <div><b>목록으로 바로 선택</b><span>지도를 누르지 않고 시·도부터 읍·면·동까지 선택할 수 있습니다.</span></div>
        ...
        <button type="button" disabled={!mapPickerDong || !mapDongChoices.includes(mapPickerDong)} onClick={() => chooseMapDong(mapPickerDong)}>{mapPickerDong ? `${mapPickerDong} 건물 가격 보기` : "동을 선택하세요"}</button>
      </div>
      <div className="map-level-tabs" aria-label="지도 확대 단계">
        <button className={`${mapFocus === "national" ? "active" : ""}${mapFocus !== "national" ? " completed" : ""}`} aria-pressed={mapFocus === "national"} onClick={() => setMapFocus("national")}><em>01</em><span>대한민국</span><small>{mapFocus === "national" ? "3D 시·도 경계" : "시·도 선택 완료"}</small></button>
        <button className={`${mapFocus === "sido" ? "active" : ""}${mapFocus === "district" || mapFocus === "buildings" || mapFocus === "detail" ? " completed" : ""}`} aria-pressed={mapFocus === "sido"} onClick={() => setMapFocus("sido")}><em>02</em><span>{selectedMapSido}</span><small>{mapFocus === "district" || mapFocus === "buildings" || mapFocus === "detail" ? "시·군·구 선택 완료" : "3D 시·군·구 경계"}</small></button>
        <button className={`${mapFocus === "district" ? "active" : ""}${mapFocus === "buildings" || mapFocus === "detail" ? " completed" : ""}`} aria-pressed={mapFocus === "district"} disabled={selectedMapSido !== activeRegion.sido} onClick={() => setMapFocus("district")}><em>03</em><span>{selectedMapSido === activeRegion.sido ? `${activeRegion.sigungu} 전체` : "시·군·구 선택"}</span><small>{mapFocus === "buildings" || mapFocus === "detail" ? "읍·면·동 선택 완료" : "3D 읍·면·동 경계"}</small></button>
      ...
      <p className="map-note">전국·시도·구·동 탐색은 <a href="https://github.com/vuski/admdongkor" target="_blank" rel="noreferrer">2026-07-01 SGIS 기반 행정구역 경계</a>(CC BY 4.0)를 사용합니다. 동을 선택한 뒤부터 네이버 실제 지도와 국토교통부 실거래 가격을 결합해 건물과 생활권을 평가합니다.</p>
    </section>
