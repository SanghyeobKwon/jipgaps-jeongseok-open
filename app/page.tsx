...
export default function Home() {
  ...
  return <main className="terminal-shell">
    ...
    <section className="map-section" id="map">
      <div className="section-title wide"><div><p>KOREA MARKET MAP</p><h2>실제 행정경계 기반 전국 지도</h2><span>{marketMonth ? `${marketMonth.slice(0, 4)}.${marketMonth.slice(4, 6)} ~ ${marketMonth.slice(-6, -2)}.${marketMonth.slice(-2)}` : "최근 3개월"} · 통계청 SGIS 시·도 경계 위에 대표 권역 분기 변화 표시</span></div><div className="map-controls"><select value={marketSort} onChange={(event) => setMarketSort(event.target.value as typeof marketSort)} aria-label="전국 시장 정렬"><option value="volume">분기 거래량 많은순</option><option value="price">분기 중위가 높은순</option><option value="rise">직전 분기 대비 상승순</option><option value="fall">직전 분기 대비 하락순</option></select><div className="map-legend"><i className="cold"/>하락 <i className="flat"/>보합 <i className="hot"/>상승</div></div></div>
      <div className="map-layout"><div className="real-korea-map"><img src={KOREA_BOUNDARY_SVG} alt="대한민국 17개 시도 실제 행정구역 경계 지도" loading="lazy" referrerPolicy="no-referrer" />{markets.map((market) => { const position = MAP_POSITIONS[market.short] || [50, 50]; return <button key={market.code} title={`${market.sido} ${market.change >= 0 ? "+" : ""}${market.change.toFixed(2)}% · 세부 지역 보기`} style={{ left: `${position[0]}%`, top: `${position[1]}%` }} className={`map-marker ${market.sido === selectedMapSido ? "selected" : ""} ${market.change > 1 ? "hot" : market.change < -1 ? "cold" : "flat"}`} onClick={() => setSelectedMapSido(market.sido)}><b>{market.short}</b><span>{market.change >= 0 ? "+" : ""}{market.change.toFixed(1)}%</span></button>})}<div className="map-compass"><i />N</div></div>
        <div className="map-ranking"><h3>전국 분기 흐름</h3><div className="ranking-labels"><span>순위</span><span>지역</span><span>중위가격</span><span>직전분기</span></div>{sortedMarkets.map((market, index) => <button key={market.code} onClick={() => setSelectedMapSido(market.sido)}><em>{String(index + 1).padStart(2,"0")}</em><b>{market.sido}<small>{market.count}건</small></b><span>{formatPrice(market.median)}</span><strong className={market.change >= 0 ? "up" : "down"}>{market.change >= 0 ? "+" : ""}{market.change.toFixed(2)}%</strong></button>)}</div>
      </div>
