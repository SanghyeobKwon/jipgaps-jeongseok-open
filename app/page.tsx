  ...
  return <main className="terminal-shell" data-view={activeSection} data-analysis-mode={analysisMode}>
    ...
    */}
    ...
    <section className="analysis-command" id="chart" aria-labelledby="analysis-command-title">
      <div className="analysis-command-head"><div><span>상세 분석</span><h1 id="analysis-command-title">{activeRegion.sido} · {activeRegion.sigungu}{selectedDong !== "all" ? ` · ${selectedDong}` : ""}{selectedProperty ? ` · ${selectedProperty.name}` : ""}</h1></div><button type="button" className="map-jump" onClick={() => { setAnalysisMode("field"); window.requestAnimationFrame(() => fieldSelectorRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })); }}><MapPin size={17} strokeWidth={1.9} aria-hidden="true" />지도에서 찾기</button></div>
      <form className="analysis-search-console" onSubmit={submitSearch}>
        ...
        <label className="analysis-address-search"><span>주소·단지·건물명</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="예: 대치동 316 또는 은마" aria-label="주소 또는 단지 건물명 검색" /></label>
