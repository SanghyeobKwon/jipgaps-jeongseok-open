...
const NAV_ITEMS = [{ id: "home", label: "지도에서 찾기", mobileLabel: "찾기" }, { id: "chart", label: "상세 분석", mobileLabel: "분석" }, { id: "research", label: "리서치", mobileLabel: "리서치" }, { id: "map", label: "전국 지도", mobileLabel: "전국" }, { id: "community", label: "커뮤니티", mobileLabel: "커뮤니티" }, { id: "policy", label: "정책", mobileLabel: "정책" }];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      fetch(`/api/trades?${params}`, { signal: controller.signal }).then(async (response) => { const data = await response.json(); if (!response.ok || data.error) throw new Error(data.error || "실거래가를 불러오지 못했습니다."); return data; }).then((data) => { setTrades(data.trades); setProperties(data.properties); if (data.properties.length === 1) setSelectedKey(data.properties[0].key); }).catch((reason) => { if (reason.name !== "AbortError") setError(publicDataErrorMessage(reason.message)); }).finally(() => { if (!controller.signal.aborted) setLoading(false); });
    }, 0);
    return () => { window.clearTimeout(timer); controller.abort(); };
  ...
  return <main className="terminal-shell" data-view={activeSection} data-analysis-mode={analysisMode}>
    ...
    */}
    ...
    <section className="monthly-board" id="market">
      ...
      <article><span>최근 3개월 거래</span><strong>{error ? "-" : latestQuarterTrades.length.toLocaleString()}{!error && <em>건</em>}</strong><small>{error ? "실거래 연결 확인 필요" : `전체 ${trades.length.toLocaleString()}건 조회`}</small></article>
      ...
      <article><span>최근 3개월 중위가격</span><strong>{error ? "-" : formatPrice(median(latestQuarterTrades.map((trade) => trade.amount)))}</strong><small>{error ? "실거래 연결 확인 필요" : "극단값 영향을 줄인 대표값"}</small></article>
    </section>
    ...
    <section className="analysis-hub" id="chart" aria-labelledby="analysis-hub-title">
      <div className="analysis-hub-copy"><h1 id="analysis-hub-title">가격과 생활 조건을 함께 확인하세요.</h1><p>같은 지역과 건물을 유지한 채 실거래 가격과 실제 생활 조건을 오갈 수 있습니다.</p></div>
      <div className="analysis-mode-switch" role="tablist" aria-label="상세 분석 방식">
        <button type="button" role="tab" aria-selected={analysisMode === "price"} className={analysisMode === "price" ? "active" : ""} onClick={() => changeAnalysisMode("price")}><Building2 size={20} strokeWidth={1.8} aria-hidden="true" /><span><b>가격·거래</b><small>실거래 차트 · 평형 비교 · 적정가격</small></span><em>가격 근거 확인</em></button>
        <button type="button" role="tab" aria-selected={analysisMode === "field"} className={analysisMode === "field" ? "active" : ""} onClick={() => changeAnalysisMode("field")}><MapPin size={20} strokeWidth={1.8} aria-hidden="true" /><span><b>생활·임장</b><small>지도 · 동선 · 시설 · 소음 · 공간</small></span><em>{FIELD_FEATURES.filter((feature) => feature.status === "live").length}개 기능 사용 가능</em></button>
      </div>
    </section>
    ...
    <section className="field-intelligence" id="field">
      <div ref={fieldSelectorRef} className="field-property-selector">
        <header><div><h3>지도에서 임장할 건물을 먼저 고르세요.</h3><p>구 경계에서 동을 누르고, 실제 도로 지도에서 건물 아이콘을 선택하면 생활 정보가 바로 연결됩니다.</p></div><div className="field-selector-current"><span>현재 선택</span><b>{selectedProperty ? selectedProperty.name : selectedDong !== "all" ? `${activeRegion.sigungu} ${selectedDong}` : `${activeRegion.sido} ${activeRegion.sigungu}`}</b><small>{selectedProperty ? `${selectedProperty.dong} · 최근 실거래 ${formatPrice(selectedProperty.lastAmount)}` : "동과 건물을 차례로 선택하세요."}</small></div></header>
        <nav className="field-selector-path" aria-label="임장 위치 선택 단계"><button type="button" className={mapFocus === "district" ? "active" : ""} onClick={() => setMapFocus("district")}><span>1</span><b>지역</b><small>{activeRegion.sigungu}</small></button><button type="button" className={mapFocus === "buildings" && !selectedProperty ? "active" : ""} disabled={selectedDong === "all"} onClick={openMapBuildings}><span>2</span><b>동</b><small>{selectedDong === "all" ? "지도에서 선택" : selectedDong}</small></button><button type="button" className={selectedProperty ? "active" : ""} disabled={!selectedProperty}><span>3</span><b>건물</b><small>{selectedProperty?.name || "아이콘 선택"}</small></button></nav>
        <div className="field-selector-layout">
          <KakaoMarketMap markets={markets} focus={mapFocus} active={activeSection === "chart" && analysisMode === "field"} propertyType={type} selectedSido={selectedMapSido} activeRegion={activeRegion} selectedDong={selectedDong} selectedBoundaryDong={selectedBoundaryDong} nearbyBoundaryDongs={nearbyBoundaryDongs} nearbyLegalDongs={nearbyLegalDongs} dongStats={mapDongStats} dongMetric={dongMetric} onDongMetricChange={setDongMetric} buildingLocations={buildingLocations} buildingsLoading={buildingsLoading} buildingsError={buildingsError} selectedPropertyKey={selectedKey} onSelectSido={chooseMapSido} onSelectRegion={chooseMapRegion} onSelectDong={chooseMapDong} onOpenBuildings={openMapBuildings} onSelectProperty={(key) => { chooseMapProperty(key); setFieldGroup("입지·동선"); setFieldFeatureId("region"); setNearbyCategory("전체"); setNearbySubtype("전체"); }} />
        ...
        <nav className="field-result-tabs" aria-label="선택 건물 임장 정보">{[
    ...
    <section className="map-section" id="map">
      <header className="map-section-title"><div><h2>전국 흐름에서 실제 생활권까지</h2><p>행정경계의 실거래 흐름을 비교하고, 동을 고른 뒤 실제 도로·건물 지도로 이어서 확인합니다.</p></div><span>{marketMonth ? `${marketMonth.slice(0, 4)}년 ${Number(marketMonth.slice(4))}월 기준 · 최근 3개월` : marketError ? "전국 실거래 연결 확인 필요" : "전국 실거래 집계 중"}</span></header>
