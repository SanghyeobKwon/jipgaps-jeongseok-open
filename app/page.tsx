...
export default function Home() {
  ...
  return <main className="terminal-shell">
    ...
    <section className="national-overview" id="national"><div><p>NATIONAL BUYING TEMPERATURE</p><h1>전국 3개월 온도판</h1><span>대표 권역의 최근 완료월과 3개월 전 가격 방향을 비교합니다.</span></div><article><span>최근 완료월 표본 거래</span><strong>{nationalDeals.toLocaleString()}<em>건</em></strong><small>{marketMonth || "집계 중"} 대표 권역</small></article><article><span>전국 중위가격</span><strong>{formatPrice(nationalMedian)}</strong><small>17개 대표 권역 중위값</small></article><article><span>3개월 전 대비</span><strong className={nationalChange >= 0 ? "up" : "down"}>{nationalChange >= 0 ? "+" : ""}{nationalChange.toFixed(2)}%</strong><small>거래량 가중 변화율</small></article><a href="#map">전국 기회 찾기 ↓</a></section>
    ...
    <section className="map-section" id="map">
      ...
      <div className="map-layout"><div className="real-korea-map"><img src={KOREA_BOUNDARY_SVG} alt="대한민국 17개 시도 실제 행정구역 경계 지도" loading="lazy" referrerPolicy="no-referrer" />{markets.map((market) => { const position = MAP_POSITIONS[market.short] || [50, 50]; return <button key={market.code} title={`${market.sido} ${market.change >= 0 ? "+" : ""}${market.change.toFixed(2)}% · 세부 지역 보기`} style={{ left: `${position[0]}%`, top: `${position[1]}%` }} className={`map-marker ${market.sido === selectedMapSido ? "selected" : ""} ${market.change > 1 ? "hot" : market.change < -1 ? "cold" : "flat"}`} onClick={() => setSelectedMapSido(market.sido)}><b>{market.short}</b><span>{market.change >= 0 ? "+" : ""}{market.change.toFixed(1)}%</span></button>})}<div className="map-compass"><i />N</div></div>
        <div className="map-ranking"><h3>전국 3개월 흐름</h3><div className="ranking-labels"><span>순위</span><span>지역</span><span>중위가격</span><span>3개월</span></div>{sortedMarkets.map((market, index) => <button key={market.code} onClick={() => setSelectedMapSido(market.sido)}><em>{String(index + 1).padStart(2,"0")}</em><b>{market.sido}<small>{market.count}건</small></b><span>{formatPrice(market.median)}</span><strong className={market.change >= 0 ? "up" : "down"}>{market.change >= 0 ? "+" : ""}{market.change.toFixed(2)}%</strong></button>)}</div>
      </div>
