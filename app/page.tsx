  ...
  return <main className="terminal-shell" data-view={activeSection} data-analysis-mode={analysisMode}>
    ...
    <section className="analysis-hub" id="chart" aria-labelledby="analysis-hub-title">
      ...
      <div className="analysis-mode-switch" role="tablist" aria-label="상세 분석 방식">
        ...
        <button type="button" role="tab" aria-selected={analysisMode === "field"} className={analysisMode === "field" ? "active" : ""} onClick={() => changeAnalysisMode("field")}><MapPin size={20} strokeWidth={1.8} aria-hidden="true" /><span><b>생활·임장</b><small>지도 · 동선 · 시설 · 소음 · 공간</small></span><em>{FIELD_FEATURES.filter((feature) => feature.status === "live").length}개 바로 사용</em></button>
      </div>
    </section>
