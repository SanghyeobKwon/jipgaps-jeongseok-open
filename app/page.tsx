    ...
    <section className="field-intelligence" id="field">
      <div ref={fieldSelectorRef} className="field-property-selector unified-map-picker" aria-label="상세 분석 지도 선택">
        <div className="field-selector-layout">
          ...
          <aside className="field-building-picker" aria-label="임장할 건물 선택"><header><span>{mapFocus === "buildings" ? "지도에 표시된 건물" : "선택 안내"}</span><b>{selectedDong === "all" ? `${activeRegion.sigungu}에서 동을 선택하세요` : `${selectedDong}과 주변 동`}</b><small>{mapFocus === "buildings" ? "아이콘 색은 지도와 같은 5단계 가격 구간이며, 건물을 바꿔도 목록은 유지됩니다." : "지도 경계를 누르면 실제 건물 단계로 이동합니다."}</small></header>{mapFocus === "buildings" ? <div>{buildingsLoading ? <p className="field-building-state">건물 좌표를 확인하고 있습니다.</p> : mapBuildingRows.length ? mapBuildingRows.slice(0, 10).map((building) => { const activeBuilding = building.key === selectedKey; return <button type="button" key={building.key} className={activeBuilding ? "active" : ""} aria-pressed={activeBuilding} onClick={() => { chooseMapProperty(building.key); setFieldGroup("입지·동선"); setFieldFeatureId("region"); setNearbyCategory("전체"); setNearbySubtype("전체"); }}><AnalysisPropertyTypeIcon type={building.propertyType} priceBucket={researchPriceBuckets(building.lastAmount)} selected={activeBuilding} size="sm" /><span><b>{building.name}</b><small>{building.dong} · {PROPERTY_MAP_META[building.propertyType].short}</small></span><strong>{activeBuilding ? formatPrice(building.lastAmount) : "선택"}</strong></button>; }) : <p className="field-building-state">{buildingsError || "좌표가 확인된 거래 건물이 없습니다."}</p>}</div> : <ol><li><span>1</span><p><b>동 경계 선택</b><small>강남구 지도에서 삼성동·대치동처럼 원하는 동을 누릅니다.</small></p></li><li><span>2</span><p><b>건물 아이콘 선택</b><small>아파트·오피스텔 등 유형 아이콘에서 임장할 건물을 고릅니다.</small></p></li><li><span>3</span><p><b>생활 정보 확인</b><small>동선·교육·의료·장보기 탭으로 바로 비교합니다.</small></p></li></ol>}</aside>
        </div>
      </div>
      <FieldScorePreview />
      ...
      <div className="map-layout"><KakaoMarketMap markets={markets} focus={mapFocus} active={activeSection === "home" || activeSection === "research"} propertyType={type} selectedSido={selectedMapSido} activeRegion={activeRegion} selectedDong={selectedDong} selectedBoundaryDong={selectedBoundaryDong} entryCamera={mapEntryCamera} dongStats={mapDongStats} dongMetric={dongMetric} onDongMetricChange={setDongMetric} buildingLocations={buildingLocations} buildingsLoading={buildingsLoading} buildingsError={buildingsError} selectedPropertyKey={selectedKey} camera={mapCamera} onCameraChange={setMapCamera} onSelectSido={chooseMapSido} onSelectRegion={chooseMapRegion} onSelectDong={chooseMapDong} onOpenBuildings={openMapBuildings} onSelectProperty={chooseMapProperty} />
        <aside className={`map-ranking${mapFocus === "buildings" ? "" : " map-selection-summary"}`} data-sheet-state={mobileSheetState} aria-label={mapFocus === "buildings" ? "선택 동과 주변 동의 최근 실거래 건물" : "선택 지역 요약"}>
          ...
          {mapFocus === "buildings" ? <>
            ...
            <div className="ranking-labels building-labels"><span>가격대</span><span>건물</span><span>최근 거래가</span><span>거래</span></div>
            {buildingStatus === "partial" && <div className="map-summary-loading" role="status">일부 주소 검증이 완료되지 않아 확인된 건물만 표시합니다.</div>}
            <div className="map-ranking-list building-list">{buildingsLoading ? <div className="ranking-loading">선택 동과 주변 동의 건물 좌표를 확인하고 있습니다.</div> : mapBuildingRows.length ? mapBuildingRows.map((building) => { const activeBuilding = building.key === selectedKey; return <button key={building.key} className={`${building.scope === "selected" ? "selected" : "nearby"}${activeBuilding ? " active-building" : ""}`} aria-pressed={activeBuilding} onClick={() => chooseMapProperty(building.key)}><AnalysisPropertyTypeIcon type={building.propertyType} priceBucket={researchPriceBuckets(building.lastAmount)} selected={activeBuilding} size="sm" /><b>{building.name}<small>{building.dong} · {PROPERTY_MAP_META[building.propertyType].short} · {building.scope === "selected" ? "선택 동" : "주변 동"}</small></b><span>{formatPrice(building.lastAmount)}</span><strong>{building.count}건</strong></button>; }) : <div className="ranking-loading">{buildingsError || "이 범위에서 지도 좌표가 확인된 거래 건물이 없습니다."}</div>}</div>
