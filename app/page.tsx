...
export default function Home() {
  ...
  return <main className="terminal-shell">
    ...
    <section className="map-section" id="map">
      <div className="section-title wide"><div><p>KOREA MARKET MAP</p><h2>실제 행정경계 기반 전국 지도</h2><span>{marketMonth ? `${marketMonth.slice(0, 4)}년 ${Number(marketMonth.slice(4))}월` : "최근 월"} · 통계청 SGIS 시·도 경계 위에 대표 권역 실거래 변화 표시</span></div><div className="map-controls"><select value={marketSort} onChange={(event) => setMarketSort(event.target.value as typeof marketSort)} aria-label="전국 시장 정렬"><option value="volume">거래량 많은순</option><option value="price">가격 높은순</option><option value="rise">상승률 높은순</option><option value="fall">하락률 높은순</option></select><div className="map-legend"><i className="cold"/>하락 <i className="flat"/>보합 <i className="hot"/>상승</div></div></div>
